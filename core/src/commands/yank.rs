pub async fn execute(backend: &impl crate::Backend, token: &str, request: &crate::models::YankRequest) -> Result<(), reqwest::Error> {
    ...
    match backend.get_file(token, &crate_path).await {
        Ok((content, encoding, sha)) => {
            ...
            if backend.update_file(token, &crate_path, &branch_name, &versions, &sha).await.is_err() {
